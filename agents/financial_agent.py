...
def financial_agent(state):
    ...
    section = load_section_map(doc_name).get("financial")

    if not section:
        return {**state, "response": "Financial section not found.", "query_type": classify_query_type(query)}
                ...
                **state,
                "intermediate_results": {
                    **state.get("intermediate_results", {}),
                    "financial": {"answer": "Not found in document.", "sources": []}
                },
    ...
    return {
        **state,
        "intermediate_results": {
            **state.get("intermediate_results", {}),
            "financial": {"answer": response.content, "sources": sources}
        },
        "query_type": query_type
    }
