...
def regulatory_agent(state):
    ...
    print("regulatory agent is running...")

    query = state["query"]
    doc_name = state["doc_name"]

    section = load_section_map(doc_name).get("regulatory")

    if not section:
        return {**state, "response": "Regulatory section not found.", "query_type": classify_query_type(query)}
            ...
            **state,
            "intermediate_results": {
                **state.get("intermediate_results", {}),
                "regulatory": {"answer": "Not found in document.", "sources": []}
            },
    ...
    return {
        **state,
        "intermediate_results": {
            **state.get("intermediate_results", {}),
            "regulatory": {"answer": response.content, "sources": sources}
        },
        "query_type": query_type
    }
